	...
	try {
		...
		if(!twilioSMS.hasOwnProperty('Body')){
		  ...
		  (!twilio.validateRequest(process.env.TWILIO_AUTH_TOKEN, event.params.header['X-Twilio-Signature'], process.env.API_GATEWAY_URL, twilioSMS)) { 
			...
			var transformedUserInputKeyedIn = twilioSMS.Body.trim();
			if(transformedUserInputKeyedIn.toLowerCase()==='k'){
				transformedUserInputKeyedIn = 'yes';
			}
				...
				var params = {
				  ...
				  inputText: transformedUserInputKeyedIn,
				  userId: userNumber,
				  sessionAttributes: {
				  }
				};
